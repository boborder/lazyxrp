...
- Toolchain version is pinned in `rust-toolchain.toml` (CI/CD and local `rustup` use the same channel). `Cargo.toml` has `rust-version` as the MSRV hint for dependents.
