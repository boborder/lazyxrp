...
3. Oracle refresh path: XRPL `get_aggregate_price` + Flare FTSOv2 (`fetch_ftso_prices`) + FXRP AssetManager C1 (`fetch_fxrp_direct_mint_info`) are polled and normalized (Flare failures non-fatal). C2 Direct Mint Payment uses Payment + 32-byte memo via `FxrpDirectMintPayment`. C3 `executeDirectMinting` is opt-in (`[flare.fassets] execute=false` by default) with pasted FDC proof JSON
...
| **Network (Flare FTSOv2 + FXRP C1/C3)** | `flare.rs` | `alloy` provider + ContractRegistry/FtsoV2 + AssetManagerFXRP reads; flagged `executeDirectMinting` |
