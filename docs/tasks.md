...
| T-012 | バイナリ配布対応（release ビルドと配布手順の整備） | DONE | P1 | `.github/workflows/cd.yml` でクロスコンパイル＋GitHub Releases 自動化。crates.io 公開も含む。`Cargo.toml` の `openssl` `vendored` で `cross` ビルド時の `openssl-sys` 不足を回避 |
