...
| T-012 | バイナリ配布対応（release ビルドと配布手順の整備） | DONE | P1 | `.github/workflows/cd.yml` でクロスコンパイル＋GitHub Releases 自動化。`workflow_dispatch` あり（入力なし）。Rust は `dtolnay/rust-toolchain@v1` で **`toolchain: stable`**（`rust-toolchain.toml` の `channel` と揃える運用）。ビルドは `build-binaries` でアーティファクト化し、**全マトリクス成功後**に `publish-github-release` と `publish-cargo` が走る。`publish-github-release` は `permissions.contents: write`（既定 read-only の `GITHUB_TOKEN` で 403 になるのを防ぐ）。crates.io 公開も含む。Linux GNU のみ `[target.'cfg(...)']` で `openssl` `vendored`（`cross` 向け）。**cross** は `use-cross` 行だけ `taiki-e/install-action` で `cross` を直接インストール（cargo-binstall なし）。**アプリ本体のビルド**は `cargo build --locked` / `cross build --locked`（ロックはリポジトリ管理）。Windows は MSVC で vendored OpenSSL+Perl を避ける |
