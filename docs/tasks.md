...
| T-012 | バイナリ配布対応（release ビルドと配布手順の整備） | DONE | P1 | `.github/workflows/cd.yml` でクロスコンパイル＋GitHub Releases 自動化。ビルドは `build-binaries` でアーティファクト化し、**全マトリクス成功後**に `publish-github-release` と `publish-cargo` が走る。crates.io 公開も含む。Linux GNU のみ `[target.'cfg(...)']` で `openssl` `vendored`（`cross` 向け）。Windows は MSVC で vendored OpenSSL+Perl を避ける |
