...
| T-012 | バイナリ配布対応（release ビルドと配布手順の整備） | DONE | P1 | `.github/workflows/cd.yml` でクロスコンパイル＋GitHub Releases 自動化。`workflow_dispatch` あり。**ツールチェイン**は `.github/scripts/resolve-rust-toolchain.sh`（`TOOLCHAIN_INPUT` / `RUST_TOOLCHAIN_FILE`）＋ composite `.github/actions/resolve-rust-toolchain` で解決し、ワークフローは二重にならない。入力 `toolchain` が空なら checkout 後に `rust-toolchain.toml` の `channel` を採用（タグ `push` でも同じ）。ビルドは `build-binaries` でアーティファクト化し、**全マトリクス成功後**に `publish-github-release` と `publish-cargo` が走る。`publish-github-release` は `permissions.contents: write`（既定 read-only の `GITHUB_TOKEN` で 403 になるのを防ぐ）。crates.io 公開も含む。Linux GNU のみ `[target.'cfg(...)']` で `openssl` `vendored`（`cross` 向け）。**cross** は `taiki-e/install-action` で **cargo-binstall** を入れたうえで `cargo binstall cross --no-confirm`（`--locked` なし）。**アプリ本体のビルド**は `cargo build --locked` / `cross build --locked`（ロックはリポジトリ管理）。Windows は MSVC で vendored OpenSSL+Perl を避ける |
