...
- **`xrpl::tests::integration_live_network`** hits `https://xrplcluster.com` (90s timeout per case) and serializes calls with a test-local mutex to reduce public-node rate limiting. Transient XRPL node errors (`insufficientNetworkMode`, `noNetwork`, `noClosed`, `noCurrent`, `tooBusy`, `failedToForward`) are retried by `rpc_value` with 1s/2s backoff. All live RPC CLI tests are `#[ignore]` because public-node availability is nondeterministic; run them explicitly with `cargo test --lib integration_live_network -- --ignored` when outbound HTTPS and live-node access are available.
