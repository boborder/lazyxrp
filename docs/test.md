...
- **`xrpl::tests::integration_live_network`** hits `https://xrplcluster.com` (90s timeout per case) and serializes calls with a test-local mutex to reduce public-node rate limiting. Transient XRPL node errors (`insufficientNetworkMode`, `noNetwork`, `noClosed`, `noCurrent`, `tooBusy`, `failedToForward`) are retried by `rpc_value` with 1s/2s backoff, so cluster routing to a lagging node no longer fails the case fast. Offline / blocked CI: non-ignored live tests fail; prefer runners with outbound HTTPS or mark live tests ignored in CI if needed.
