...
> **When this file is mandatory:** Any `EventStream`, `tokio`, `select!`, background
> work, ticks, or networked TUI — see the routing table in `SKILL.md`. Do not skim.

## NEVER / footguns (async + terminal)

| Do not | Why |
|--------|-----|
| Single-arm `select!` forever (only `events.next()`) | Other futures (tick, shutdown, IO) never run; resize/resize-handling stalls. |
| Busy-loop `terminal.draw` without gating | Pegs CPU; pair draws with events or `needs_render` + min frame time (see below). |
| Block the runtime (`std::thread::sleep`, disk on main task) | Freezes input; use `tokio::time::sleep`, `spawn_blocking`, or offload. |
| Forget `crossterm` `event-stream` feature | `EventStream` integration expects the feature flag in sync with docs/examples. |
| `unwrap` on `mpsc::send` for user-visible errors | Drops errors silently; log or surface in UI. |

**Ordering:** If one `select!` branch is *always* ready, others can starve. Mitigations:
prioritize branches, combine tick with work counters, or use `tokio::sync::Notify` /
fair channels — see *Multiple Event Sources* below.
