...
> **Routing:** Full TEA, components, modals, and action wiring live here. `SKILL.md`
> stays short — load this file whenever you design structure (not just copy-paste one
> widget).

## Layout footguns (ratatui 0.30+)

- **Zero-sized `Rect`:** competing `Constraint`s can yield 0×0 areas; skip render or
  show a fallback when `width == 0` or `height == 0`.
- **`Fill` chains:** `Fill` only receives space after fixed `Length` / `Min` claims;
  shrinking the parent to `Length(0)` collapses nested `Fill` children.
- **Stateful widgets:** passing a **cloned** `ListState` to `render_stateful_widget`
  breaks selection — keep one authoritative state object per list.
...
pub trait Component {
    ...
    /// Render the component (`&mut self` so stateful widgets get real `ListState`, not clones)
...
7. **Modal routing first** - if a modal/dialog is open, handle that subtree before global
   keys so `q` cannot quit underneath a confirmation.
8. **Single source of `ListState` / `TableState`** - render with `&mut` to the same
   struct you mutate in `handle_key`; never `clone()` for `render_stateful_widget`.
