...
Generate bindings for your contract before calling `DeployMyContract` (replace module and paths to match your project):

```bash
# Compile Solidity to get ABI + bytecode
solc --evm-version cancun --abi --bin MyContract.sol -o build/

# Generate Go bindings (DeployMyContract, NewMyContract, etc.)
abigen \
  --abi build/MyContract.abi \
  --bin build/MyContract.bin \
  --pkg coston2 \
  --type MyContract \
  --out coston2/MyContract.go
```
...
import (
    ...
    "your-module-name/coston2" // generated by abigen above
)
...
func deployContract() {
    ...
    address, tx, _, _ := coston2.DeployMyContract(auth, client)
